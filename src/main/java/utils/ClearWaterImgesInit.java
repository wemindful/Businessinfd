...
public class ClearWaterImgesInit {
    ...
    public ClearWaterImgesInit(BufferedImage image) {
        this.image = image;
        iw = image.getWidth();
        ih = image.getHeight();
    ...
    /**
     *
     * @param bi 待转换的bufferImge
     * @return 去除水印后的bufferimage
     */
    public static BufferedImage fileToclearWatermark(BufferedImage bi){
        ClearWaterImgesInit flt = new ClearWaterImgesInit(bi);
        flt.changeGrey();
        flt.getGrey();
        flt.getBrighten();
        return  bi = flt.getProcessedImg();
    }
}
